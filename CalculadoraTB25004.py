...
COLOR\_FONDO = "#f0f0f0"
COLOR\_BOTON = "#d9d9d9"
COLOR\_ENTRADA = "#ffffff"

def agregar\_caracter(caracter):
...
ventana.configure(bg=COLOR\_FONDO)
...
entrada\_entry = tk.Entry(
ventana, textvariable=entrada, font=("Arial", 24),
bd=10, bg=COLOR\_ENTRADA, relief=tk.FLAT, justify="right"
)
entrada\_entry.grid(row=0, column=0, columnspan=4, sticky="nsew", padx=10, pady=10)
...
comando = acciones.get(texto, lambda t=texto: agregar\_caracter(t))
boton = tk.Button(
ventana, text=texto, font=FUENTE,
bg=COLOR\_BOTON, relief="raised", borderwidth=1,
command=comando
)
